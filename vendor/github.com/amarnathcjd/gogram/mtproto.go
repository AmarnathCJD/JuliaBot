...
func (m *MTProto) connect(ctx context.Context) error {
	...
	// on ctx cancel, close the connection
	// go func() {
	// 	<-ctx.Done()
	// 	_ = m.transport.Close()
	// }()
...
func (m *MTProto) stopRoutines() {
	for _, f := range m.ctxCancel {
		f()
		for i, v := range m.ctxCancel {
			if reflect.DeepEqual(f, v) {
				m.ctxCancel = append(m.ctxCancel[:i], m.ctxCancel[i+1:]...)
				break
			}
		}
	}
...
// 			fmt.Println("Active READERS connections:", activeReads)
// 		}
// 	}()
// }
