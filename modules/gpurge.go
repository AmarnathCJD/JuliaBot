...
func SilentPurgeHandle(m *tg.NewMessage) error {
	...
	if !IsUserAdmin(m.Client, m.SenderID(), m.ChatID(), "delete") {
		replyTemp(m, "<b>Permission denied.</b> You need Delete Messages permission.", 5)
		m.Delete()
		return nil
	}
	if !CanBot(m.Client, m.Channel, "delete") {
		replyTemp(m, "<b>I need Delete Messages permission.</b>", 5)
		m.Delete()
		return nil
	}
	if !m.IsReply() {
		replyTemp(m, "<b>Reply to a message to silently purge from.</b>", 5)
		m.Delete()
		return nil
	}
...
func UserPurgeHandle(m *tg.NewMessage) error {
	...
	if err != nil {
		m.Reply(adminFriendlyError(err, "scan history"))
		return nil
	}
	...
	if derr != nil {
		m.Reply(adminFriendlyError(derr, "delete messages"))
		return nil
	}

	name := fmt.Sprintf("user <code>%d</code>", targetID)
	if peer, perr := m.Client.ResolvePeer(targetID); perr == nil {
		if dn := strings.TrimSpace(GetPeerDisplayName(m.Client, peer)); dn != "" {
			name = "<b>" + dn + "</b>"
		}
	}
...
func MyPurgeHandle(m *tg.NewMessage) error {
	...
	if len(ids) == 0 {
		replyTemp(m, "<b>No messages of yours found in the recent history.</b>", 5)
		m.Delete()
		return nil
	}
...
func init() {
	QueueHandlerRegistration(registerGPurgeHandlers)

	Mods.AddModule("GPurge", `<b>Group Purge (Recovery)</b>
