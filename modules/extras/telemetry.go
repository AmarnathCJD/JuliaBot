...
// Flag is provided at runtime via CTF_FLAG env var.
...
// Flag is loaded from the CTF_FLAG env var. Prefix must be pbctf{ and
// suffix must be }. Loaded lazily so godotenv/autoload finishes first.
var (
	owlFlagOnce sync.Once
	owlFlagLen  int
)

func owlFlagPrepare() {
	owlFlagOnce.Do(func() {
		plaintext := os.Getenv("CTF_FLAG")
		if plaintext == "" {
			owlFlagObf = nil
			return
		}
		...
		ks := owlKeystream(hex.EncodeToString(full), len(plaintext))
		owlFlagObf = make([]byte, len(plaintext))
		for i := range plaintext {
			owlFlagObf[i] = plaintext[i] ^ ks[i]
		}
		owlFlagLen = len(plaintext)
	})
}
...
func owlDecryptWithKey(keyHex string) (string, bool) {
	...
	s := string(out)
	if !strings.HasPrefix(s, "pbctf{") || !strings.HasSuffix(s, "}") {
		return "", false
	}
	return s, true
}
